...
def make_hits_around_position_and_get_total(x, y, width, height):
    this_total = 0
        ...
        for j in range(max(y - 1, 0), min(y + 2, height)):
            ...
            if (i, j) != (x, y):
                ...
                if (i, j) in numbers_dict:
                    ...
                    full_number_value = get_full_num_and_mark_all_hits_from_pos(start_num_x, start_num_y)
                    this_total += full_number_value
    return this_total
...
def do_it(file_path: str) -> {}:
    ...
    for symbol_pos in symbols_pos_list:
        x, y = symbol_pos
        total += make_hits_around_position_and_get_total(x, y, width, height)
